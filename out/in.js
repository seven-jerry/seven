...
(function (seven) {
    class View extends seven.JBObject {
        constructor() {
            super();
            this.appliadFrame = seven.Rect.empty();
            this.sizeKey = "";
            this.useBaseClass = undefined;
            this.constraints = new Array();
            this.strokewWidth = 1;
            this.hidden = false;
            this.zIndex = undefined;
            this.padding = seven.Rect.empty();
            this.margin = seven.Rect.empty();
            this.dragable = false;
            ...
            */
            ...
            this.hasLoggedRefFrame = false;
            this.tag = "";
            this.subViews = new Array();
            this.referenceFrame = seven.Rect.empty();
            this.initialReferenceFrame = seven.Rect.empty();
            this.totalOffset = seven.Orgin.empty();
            this.style = new seven.ViewStyle().initWithView(this);
        }
        ...
        initWithReferenceFrame(frame) {
            this.initialReferenceFrame = seven.Rect.copyRect(frame);
            this.referenceFrame = seven.Rect.copyRect(frame);
            return this;
        }
        /**
        *
        ...
        */
         ...
         */
        setupFrameFromKey() {
            if (seven.JBDocument.document().getSizeLoader() == undefined) {
                seven.Logger.develepor("View : setupFrameFromKey : JBDocument.document().getSizeLoader() falied");
                return;
            }
            var frame = seven.JBDocument.document().getSizeLoader().getSizeRectForKey(this.sizeKey);
            if (frame != undefined) {
         ...
         */
        changeReferenceFrame(frame) {
            this.referenceFrame = seven.Rect.copyRect(frame);
            if (this.hasLoggedRefFrame == false) {
                this.hasLoggedRefFrame = true;
                seven.Logger.develeporInfo(this.getClassName() + " - changeReferenceFrame  : Generic signature used." +
         ...
         */
        buildAppliadFrame() {
            this.appliadFrame.setX(seven.JBDocument.document().horizontalScreenValue(this.referenceFrame.x(), this.shouldUseBaseClass()));
            this.appliadFrame.setY(seven.JBDocument.document().verticalScreenValue(this.referenceFrame.y(), this.shouldUseBaseClass()));
            this.appliadFrame.setWidth(seven.JBDocument.document().horizontalScreenValue(this.referenceFrame.width(), this.shouldUseBaseClass()));
            this.appliadFrame.setHeight(seven.JBDocument.document().verticalScreenValue(this.referenceFrame.height(), this.shouldUseBaseClass()));
            for (let subView of this.subViews) {
                subView.buildAppliadFrame();
            }
            ;
        ...
        copyAllSubViews(destinationView) {
            this.recursiveCopySubviews(destinationView);
        }
        recursiveCopySubviews(destinationView) {
            for (let subView of this.subViews) {
         ...
         */
        applyActionToSubViews(layers, action) {
            this.applyActionToAllSubViews(layers, action);
        }
        applyActionToAllSubViews(layers, action) {
            action(this);
            if (layers == 0) {
                return;
            }
        ...
        getViewForOrignAndOptions(orgin, options) {
            ...
            buildArray = buildArray.sort((a, b) => {
        ...
        getViewsForOrgin(buildArray, orgin) {
            this.subViews = this.subViews.sort((a, b) => {
                return b.getZIndex() - a.getZIndex();
            });
        ...
        apliadOffset() {
            ...
            return seven.Orgin.copyOrgin(orgin);
        }
        referenceOffset() {
            var orgin = this.screenOffset();
            if (orgin.isEmpty()) {
            ...
            var x = seven.JBDocument.document().getResizeManager().horizontalReferenceValue(orgin.x, this.shouldUseBaseClass());
            var y = seven.JBDocument.document().getResizeManager().verticalReferenceValue(orgin.y, this.shouldUseBaseClass());
            return new seven.Orgin(x, y);
        }
        screenOffset() {
            var returnOrgin = new seven.Orgin(0, 0);
            return this.buildScreenOffset(returnOrgin);
        }
        buildScreenOffset(orgin) {
...
var seven;
(function (seven) {
    class ViewStyle extends seven.JBObject {
        constructor() {
            super(...arguments);
            this.constraints = new seven.JMBMap();
        }
        initWithView(view) {
            this.view = view;
            return this;
        ...
        set minHeight(minHeight) {
            var minHeightConst = new seven.MinHeight().init(minHeight);
            this.constraints.put("3", minHeightConst);
        }
        set leading(marginLeft) {
            var marginLeftConstraint = new seven.LeadingContraint().init(marginLeft);
            this.constraints.put("0", marginLeftConstraint);
        }
        set trailing(marginRight) {
            var marginRightConstraint = new seven.TraillingContraint().init(marginRight);
            this.constraints.put("1", marginRightConstraint);
        }
        marginTop(top, toView) {
            var topConstraints = new seven.Top().init(top, toView);
